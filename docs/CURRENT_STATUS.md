...
**Random events phase one: framework and Supply Drop — Pass for automated lifecycle and isolation; human pacing remains Partial.** `SurvivalEventDirector` now registers only `supply_drop`, schedules it once per run on seeded wave 3/5/7, and exposes explicit IDLE/PENDING/ACTIVE/RESOLVING/COOLDOWN state. Medical restores 30% max health, Weapon refills all four magazines, and Tactical adds one bounded grenade plus full stamina; full resources convert to score. The event replaces that wave's ordinary field cache, pauses WaveManager and Sublevel steam, and uses paired reason-based control locks shared with upgrade selection. Settlement records the trigger wave, choice, actual health, magazine and grenade gains. The test forces all three resource paths, mouse/digit selection, full-resource replacement, pause/double-select protection, Sublevel movement recovery, two full ten-wave maps, death reset and PVE isolation. Elite Bounty and Emergency Reinforcements are intentionally not registered in this phase.
...
SURVIVAL_EVENTS_PASS supply-only scheduling, resource choices, input cleanup, two-map completion and PVE isolation
