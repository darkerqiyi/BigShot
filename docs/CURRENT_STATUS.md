...
**Run-local random events and supply choices — Pass for automated lifecycle and isolation; human pacing remains Partial.** `SurvivalEventDirector` uses map/wave/weight/duration/cooldown configuration to seed at most two distinct events across waves 3/5/7. Supply Drop pauses the next-wave flow only after a clear and offers real-value medical/ammo/tactical cards with full-resource score fallbacks. Elite Bounty adds one existing marked elite with a 26-second one-shot success/failure result. Emergency Reinforcements lasts at most 24 seconds, uses 75% spawn interval, +1 active cap, and at most one existing elite before restoring the captured wave values. Events never overlap waves 2/4/6/8 upgrades or wave 10, pause with the tree, suspend Sublevel steam during post-wave selection, and clear on death/restart/menu/Boss. Settlement records event names, outcomes, bounty/reinforcement success counts, and actual supply types. `survival_events_test.gd` forces all three paths, bounty success/failure, two distinct seeded events, two full ten-wave maps, death reset, and PVE isolation.
...
SURVIVAL_EVENTS_PASS deterministic two-event scheduling, supply choice, bounty success/failure, bounded reinforcements, two-map completion, reset and PVE isolation
