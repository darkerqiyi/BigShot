...
**Survival acceptance and pacing pass — Pass for automated stability/pressure; Partial for human duration.** The old uniform 55-enemy rush is now a 77-enemy, per-wave-configured sequence with 3→6 active caps, 2–5 unit reinforcement batches, 4–8 second overlapping reinforcement cadence, one early attack slot and two later slots, occupied-spawn rejection, and supplies after waves 3/5/7/9. Debug-only `SurvivalBalanceTelemetry` records each wave's timing, composition, pressure, incoming damage, resource boundaries, weapon contribution, and grenade use. Fixed-seed real-projectile automation settles once in 268.5 seconds using all four weapons (rifle/shotgun/sniper/pistol damage 1948/1904/3036/800) and in 274.2 seconds using rifle only plus 228 successful roll inputs; the latter still records seven projectile dodges and 437 incoming damage, so repeating roll does not create permanent immunity. These bots deliberately use near-perfect aim and heal between frames to preserve damage-source evidence, so the requested human 8–15 minute clear and 2–4 minute Boss fight remain unverified. Upgrade implementation has proceeded behind isolated runtime boundaries, but its final balance remains blocked on those human sessions.

**Lightweight in-run Roguelite upgrade loop — Pass for implementation and automated isolation; Partial for human Build balance.** Waves 2/4/6/8 now pause the survival intermission for one three-card choice drawn from twelve capped, weighted definitions. `RunUpgradeManager` recomputes final player, grenade, and instance-local weapon values from immutable baselines, while the card UI only requests a choice and never edits gameplay values. Maxed options leave the pool; roll, stamina, reload, pellet, penetration, grenade, health, and sprint values retain explicit safety bounds. Death, restart, exit, debug reload, and PVE entry restore base values. The focused gate applies all 32 legal stacks and validates input/pause/reset behavior; deterministic mixed and rifle-only ten-wave runs complete in 270.69 and 276.32 simulated seconds with four choices each. Human choice quality, card readability under real input, and dominant-Build risk remain unverified.
...
- Four weapon signatures are deliberately different: rifle is a short controlled mid transient with a restrained action layer; shotgun is the widest low/noisy burst plus a delayed pump; rail lance is the longest sharp high-energy shot plus bolt action; sidearm is short, higher, and quieter with a fast slide. Each weapon has a distinct reload start, common insert/complete stages, and an empty click. Heavy fire remains above sustained/light fire without changing weapon timing.
...
PROTOTYPE_VERIFY_PASS PVE regression, survival 3/10-wave/upgrade loops, mode selection, abilities, scripted playthroughs, and 120-frame runtime checks passed
```
...
2. Run two human ten-wave survival sessions with the upgrade overlay enabled; record clear/death time, every offered/selected card, rejected cards, weapon use, supply sufficiency, and whether selection interrupts pacing comfortably.
3. Use those sessions to tune selection weights and weak/mandatory cards, then deepen only the proven Build interactions without adding permanent progression or coupling upgrades to PVE defaults.
