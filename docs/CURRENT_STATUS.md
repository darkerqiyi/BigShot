...
**Survival acceptance and pacing pass — Pass for automated stability/pressure; Partial for human duration.** The curve now contains 118 configured enemies before Boss support, with staged role introductions, elites on waves 6/8/9, 4→7 active caps, 3–6 unit reinforcement batches, 0.38–0.58-second individual spawn spacing, one early attack slot and two later slots, occupied-spawn rejection, and bounded supplies. Normal rests are 3.5 seconds, upgrade completion resumes after about two seconds, and waves 5/9 retain 4.5/5.0-second supply windows; fast-start still preserves the final spawn warning. Debug-only `SurvivalBalanceTelemetry` records each wave's combat duration, actual wait, composition, pressure, incoming damage, resources, weapon contribution/headshots, supplies, and upgrade. Measured pre-change runs were 635.91 seconds mixed (with a 241.92-second outlier wave) and 328.19 seconds rifle-only. Post-change real-projectile automation completed mixed routes in 364.12–436.31 seconds and rifle-only in 474.46 seconds, always peaking at two attackers and seven active enemies. Because these bots heal every frame and produce roughly 46–55% headshots, the intended 8–12 minute human window remains unverified.

**Lightweight in-run Roguelite upgrade loop — Pass for implementation and automated isolation; Partial for human Build balance.** Waves 2/4/6/8 pause spawning for one three-card choice drawn from twelve capped, weighted definitions. Candidate generation now supplies output, survival/resource, and mobility/function families when available, never duplicates a card, excludes maxed entries, and displays the exact before/after runtime value. `RunUpgradeManager` still recomputes final player, grenade, and instance-local weapon values from immutable baselines; roll, stamina, reload, pellet, penetration, grenade, health, and sprint retain explicit safety bounds. Death, restart, exit, debug reload, and PVE entry restore base values. Full mixed and rifle-only runs each apply exactly four choices without state leakage. Human choice quality and dominant-Build risk remain unverified.

**Survival run settlement statistics — Pass for automated consistency.** Victory and failure summaries now report reached wave, survival time, total kills, total applied weapon-plus-grenade damage, damage received, headshot count/rate, most-used weapon by equipped time, per-weapon kills, selected Build, and Boss duration when applicable. `RunTelemetry` records headshots alongside real applied damage, while the survival-only summary reads that in-memory snapshot; no data is sent, persisted as telemetry, or exposed to the PVE HUD. Debug wave summaries additionally record actual intermission waits and upgrade selections.

**Survival durability, headshots, damage numbers, and tighter deployment — Pass for automated gates; human readability/balance remains Partial.** Survival base health is 132/120/216/900 for assault/gunner/shield/elite and grows linearly by role (+8/+6/+9/+28 per later wave); PVE remains 44/58/92/230. Visual-following head areas retain the centralized 2.0 multiplier without changing physics, shield fronts, weapon damage, or Boss core rules. A fixed 64-label pool shows actual final damage, priority headshots, and block results. Waves 1–9 contain 6/8/9/10/15/13/15/18/24 enemies with 4→7 active caps. The result removes the old single-wave stall as the sole source of target duration, but high-accuracy automated runs still finish below the 8-minute human target.
...
**Second survival map and map configuration modularization — Pass for automated and rendered gates; human arena-flow acceptance remains Partial.** The original industrial arena remains selectable and unchanged in identity. A new two-card menu also loads `SUBLEVEL-09 // 废弃地下运输站`, whose 1600×720 configured arena adds reachable low upper lanes, ground/upper spawn groups, map-local Boss bounds, a compact six-enemy cap, and a warned steam vent. `SurvivalMapConfig` now supplies player/enemy/Boss spawns, camera bounds, platform/hazard definitions, art identity, and small pacing offsets; the project still owns one WaveManager, one ten-wave definition, one upgrade manager, and one survival controller. The steam cycle uses 4.5s initial delay / 1.25s warning / 0.95s active / 6.2s cooldown, deals 18 environment damage to the player and 24 to ordinary enemies, remains dangerous during roll, and suspends across rests, upgrade choice, death, settlement, and Boss intro. Both maps complete accelerated ten-wave/Boss/upgrade automation once each, and Sublevel-09 passes same-map death restart plus platform jump reachability. A real 1280×720 OpenGL/Metal capture is stored at `/tmp/bigshot-sublevel09.png`; subjective spawn pressure, steam cadence, and repeated human clear flow remain to be judged.

**Expanded random events — Pass for automated lifecycle and isolation; human pacing remains Partial.** `SurvivalEventDirector` now schedules up to two distinct seeded events among Supply Drop, Elite Bounty, and Emergency Reinforcements on waves 3/5/7, with one occurrence per type and explicit IDLE/PENDING/ACTIVE/RESOLVING/COOLDOWN state. Supply retains its safe post-wave three-card resource flow. Bounty marks one existing elite for 26 seconds and awards 750 score plus a small useful resource on one-shot success; timeout removes the mark without blocking the wave. Reinforcements last up to 24 seconds, temporarily use a 0.75 spawn-interval multiplier and +1 active slot, add at most one existing elite, and award 600 score plus a small useful resource. Pause suspends timers, and death/restart/Boss/menu transitions remove markers, modifiers, UI, timers, and event state. The expanded test covers all supply paths, bounty success/failure/same-frame uniqueness, paused reinforcement timing, two event-death restarts, both full ten-wave maps, no upgrade overlap, Boss cleanup, settlement statistics, and PVE isolation.

**Demo product-flow, persistent settings, and first-run guidance — Pass for automated gates.** Cold start now follows title → main menu → mode/map selection → fade transition → map identity/objective → three-second deployment countdown. Pause exposes continue, guarded restart, settings, controls, and guarded return-to-menu; settlement exposes replay, map reselection, and main menu. `SettingsManager` applies and persists display, Master/Music/SFX/UI mix, experience options, and supported key bindings in `user://settings.cfg`, rejects duplicate critical keys, and safely repairs missing/corrupt values. The compact tutorial advances only after each real action and can be reset from settings.

**Five-route balance and performance acceptance — Pass for scripted gates; human feel remains Partial.** All five requested ten-wave routes reached settlement: industrial rifle 459.96s, industrial baseline 657.71s, industrial grenade/mobility 736.71s, Sublevel shotgun 786.98s, and Sublevel sniper 773.78s. Every route produced 119 kills; industrial peak active enemies stayed at seven and Sublevel at six, while Boss duration remained 10.68–16.33s. Test bots provide repeatable timing/state/pressure evidence rather than human difficulty evidence. Headless process shutdown still reports 14–16 ObjectDB instances, while scene-flow/reset gates show no in-session state carry-over; long repeated graphical sessions remain an explicit profiling item.

**Release-candidate packaging — Partial.** Version `0.9.0-demo`, clean cold-start defaults, updated run/export documentation, `docs/RELEASE_QA.md`, and macOS/Windows export presets are present. Both real export commands reached their presets but failed only because the local Godot 4.7 installation has no templates; no package is claimed or left in `builds/`. macOS notarization and Windows signing also remain distribution-owner steps.
...
SURVIVAL_EVENTS_PASS two-event scheduling, supply choices, bounty success/failure, reinforcements, cleanup, two-map completion and PVE isolation
