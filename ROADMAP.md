...
- Waves 2, 4, 6, and 8 enter a dedicated `UPGRADE_SELECTION` state after danger cleanup and before a two-second post-choice countdown. Wave 10 goes directly to settlement. Candidate cards are unique, exclude maxed upgrades, and deliberately span output, survival/resource, and mobility/function families while the pool permits.
- The screen-space three-card overlay accepts mouse, 1/2/3, and keyboard focus plus Enter. Each card shows the actual current-to-next runtime value (for example `ROLL 0.50s -> 0.42s`). While it is open, player actions and weapon switching are suppressed, enemy generation is stopped, and the existing pause menu remains authoritative.
...
- Survival victory/failure settlement now reads the same in-memory run snapshot to show reached wave, time, kills, total dealt/received damage, headshot count/rate, most-used weapon, per-weapon kills, Build, and Boss duration. PVE never loads this run manager or survival settlement state.
...
- The measured pre-change routes were 635.91 seconds mixed (including one 241.92-second shield/positioning outlier) and 328.19 seconds rifle-only. After the linear durability/composition pass, the full real-projectile routes complete in 364.12–436.31 seconds mixed and 474.46 seconds rifle-only, peak at seven active/two attacking enemies, settle with four valid choices, and retain no persistent upgrade state. These invulnerable high-accuracy bots remain faster and more variable than the 8–12 minute human target.
...
- `scripts/config/enemy_balance.gd` centralizes mode-aware durability. PVE retains the authored 44/58/92/230 health values. Survival now starts at 132/120/216/900 for assault/gunner/shield/elite and adds a role-specific linear +8/+6/+9/+28 per completed wave index; no behavior script or shared Resource is duplicated and no exponential scaling is used.
