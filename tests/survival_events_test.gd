...
func _run() -> void:
	...
	await _verify_combat_event_death_reset()
	...
	var industrial_random := await _complete_random_event_run(IndustrialScene, &"industrial_district", 6301)
	var sublevel_random := await _complete_random_event_run(SublevelScene, &"sublevel_09", 6302)
	...
	_expect(int(industrial_random.get("events", 0)) == 2 and int(sublevel_random.get("events", 0)) == 2, "a random run did not resolve exactly two events")
	_expect(int(industrial_random.get("unique_events", 0)) == 2 and int(sublevel_random.get("unique_events", 0)) == 2, "a random run repeated an event type")
	_expect(not bool(industrial_random.get("overlap", true)) and not bool(sublevel_random.get("overlap", true)), "random event and upgrade UI overlapped")
	print("SURVIVAL_EVENTS_METRICS supply_industrial=%s supply_sublevel=%s random_industrial=%s random_sublevel=%s" % [industrial, sublevel, industrial_random, sublevel_random])
	_finish()
...
func _verify_director_contract() -> void:
	...
	for definition in definitions:
		definition_ids[StringName(definition.get("event_id", &""))] = true
		_expect(bool(definition.get("debug_force_enabled", false)), "event cannot be forced in debug")
	_expect(definition_ids.size() == 3 and definition_ids.has(&"supply_drop") and definition_ids.has(&"elite_bounty") and definition_ids.has(&"emergency_reinforcements"), "event registry is incomplete")
	var supply := EventData.get_event(&"supply_drop")
	_expect(bool(supply.get("blocks_wave_start", false)) and bool(supply.get("pauses_combat", false)), "supply definition omitted blocking/combat flags")
	...
	_expect(first.schedule.size() == 2, "director did not schedule exactly two events")
	var scheduled_ids: Dictionary = {}
	if not first.schedule.is_empty():
		for wave_value in first.schedule:
	...
	_expect(scheduled_ids.size() == 2, "director scheduled a duplicate event type")
	_expect(first.debug_force_event(&"elite_bounty", 3), "bounty event is not debug-triggerable")
	first.enabled = false
	first.reset_run()
...
func _complete_supply_run(scene: PackedScene, expected_map: StringName, seed_value: int) -> Dictionary:
	...
	_expect("DROP WAVE" in game.hud.state_subtitle.text, "%s settlement omitted supply statistics" % expected_map)
	game.queue_free()
	await process_frame
	await process_frame
	return result
...
func _verify_bounty(expect_success: bool) -> void:
	var game := _new_game(IndustrialScene if expect_success else SublevelScene, 2101 if expect_success else 2102)
	...
	if target != null:
		if expect_success:
			...
			_expect(not game.debug_expire_survival_event(), "resolved bounty could still expire in the same frame")
		else:
...
func _verify_reinforcements() -> void:
	var game := _new_game(IndustrialScene, 2201)
	...
	for _frame in range(2600):
		...
		if game.event_director.is_active(&"emergency_reinforcements"):
			active_seen = true
			modifier_seen = modifier_seen or game.wave_manager.event_modifiers_active()
			var remaining_before: float = game.event_director.remaining
			paused = true
			await create_timer(0.04).timeout
			_expect(is_equal_approx(game.event_director.remaining, remaining_before), "reinforcement timer advanced while paused")
			paused = false
		for enemy in game.enemies.get_children():
	...
	_expect(not game.wave_manager.event_modifiers_active(), "reinforcement modifiers remained after resolution")
	game.queue_free()
	await process_frame
	await process_frame


func _verify_combat_event_death_reset() -> void:
	var game := _new_game(SublevelScene, 2251)
	_force_only(game, &"emergency_reinforcements", 3)
	for _frame in range(2200):
		_handle_upgrade(game)
		_kill_all_hostiles(game)
		if game.event_director.is_active(&"emergency_reinforcements"):
			break
		await physics_frame
	_expect(game.event_director.is_active(&"emergency_reinforcements"), "combat-event death reset did not reach reinforcement")
	var old_id := game.get_instance_id()
	game.player.take_damage(9999, Vector2.ZERO, game.player.global_position, {"source": &"event_reset_test", "damage_kind": &"environment"})
	await create_timer(1.8).timeout
	_expect(current_scene != null and current_scene.get_instance_id() != old_id, "death during reinforcement did not reload")
	if current_scene != null and current_scene.get_instance_id() != old_id:
		_expect(current_scene.event_director.get_history().is_empty() and not current_scene.event_director.is_active(), "restart retained combat-event state")
		_expect(not current_scene.wave_manager.event_modifiers_active(), "restart retained reinforcement modifiers")
		_expect(current_scene.player.controls_enabled and current_scene._control_locks.is_empty(), "restart retained combat-event input lock")
		current_scene.queue_free()
		await process_frame


func _complete_random_event_run(scene: PackedScene, expected_map: StringName, seed_value: int) -> Dictionary:
	var game := _new_game(scene, seed_value)
	var scheduled_ids: Dictionary = {}
	...
	_expect(game.map_id == expected_map, "%s random event run loaded the wrong map" % expected_map)
	_expect(bool(result["complete"]), "%s random event run did not complete" % expected_map)
	_expect(bool(result["boss_clean"]), "%s random event run retained Boss or projectile state" % expected_map)
	game.queue_free()
	await process_frame
	await process_frame
	return result
...
func _finish() -> void:
	...
	if failures.is_empty():
		print("SURVIVAL_EVENTS_PASS two-event scheduling, supply choices, bounty success/failure, reinforcements, cleanup, two-map completion and PVE isolation")
		quit(0)
	else:
