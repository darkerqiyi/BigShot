...
if ! "$GODOT_BIN" --headless --path "$ROOT" --script res://tests/player_sprint_stamina_test.gd >"$LOG_DIR/player_sprint_stamina.log" 2>&1; then
	cat "$LOG_DIR/player_sprint_stamina.log" >&2
	exit 1
fi
filtered_errors="$(grep -E "SCRIPT ERROR|Parse Error|Failed to load script|ERROR:" "$LOG_DIR/player_sprint_stamina.log" | grep -v 'get_system_ca_certificates' | grep -v 'Condition "ret != noErr"' || true)"
if [[ -n "$filtered_errors" ]]; then
	printf '%s\n' "$filtered_errors" >&2
	exit 1
fi
grep "PLAYER_SPRINT_STAMINA_PASS" "$LOG_DIR/player_sprint_stamina.log"

if ! "$GODOT_BIN" --headless --path "$ROOT" --script res://tests/sprint_jump_momentum_test.gd >"$LOG_DIR/sprint_jump_momentum.log" 2>&1; then
	cat "$LOG_DIR/sprint_jump_momentum.log" >&2
	exit 1
fi
filtered_errors="$(grep -E "SCRIPT ERROR|Parse Error|Failed to load script|ERROR:" "$LOG_DIR/sprint_jump_momentum.log" | grep -v 'get_system_ca_certificates' | grep -v 'Condition "ret != noErr"' || true)"
if [[ -n "$filtered_errors" ]]; then
	printf '%s\n' "$filtered_errors" >&2
	exit 1
fi
grep "SPRINT_JUMP_MOMENTUM_PASS" "$LOG_DIR/sprint_jump_momentum.log"
...
if ! "$GODOT_BIN" --headless --path "$ROOT" --script res://tests/mode_select_test.gd >"$LOG_DIR/mode_select.log" 2>&1; then
	cat "$LOG_DIR/mode_select.log" >&2
	exit 1
fi
filtered_errors="$(grep -E "SCRIPT ERROR|Parse Error|Failed to load script|ERROR:" "$LOG_DIR/mode_select.log" | grep -v 'get_system_ca_certificates' | grep -v 'Condition "ret != noErr"' || true)"
if [[ -n "$filtered_errors" ]]; then
	printf '%s\n' "$filtered_errors" >&2
	exit 1
fi
...
filtered_errors="$(grep -E "SCRIPT ERROR|Parse Error|Failed to load script|ERROR:" "$LOG_DIR/survival_three.log" | grep -v 'get_system_ca_certificates' | grep -v 'Condition "ret != noErr"' || true)"
if [[ -n "$filtered_errors" ]]; then
	printf '%s\n' "$filtered_errors" >&2
	exit 1
fi
...
if ! "$GODOT_BIN" --headless --path "$ROOT" --script res://tests/survival_headshot_pacing_test.gd >"$LOG_DIR/survival_headshot.log" 2>&1; then
	cat "$LOG_DIR/survival_headshot.log" >&2
	exit 1
fi
filtered_errors="$(grep -E "SCRIPT ERROR|Parse Error|Failed to load script|ERROR:" "$LOG_DIR/survival_headshot.log" | grep -v 'get_system_ca_certificates' | grep -v 'Condition "ret != noErr"' || true)"
if [[ -n "$filtered_errors" ]]; then
	printf '%s\n' "$filtered_errors" >&2
	exit 1
fi
grep "SURVIVAL_HEADSHOT_PACING_PASS" "$LOG_DIR/survival_headshot.log"
...
filtered_errors="$(grep -E "SCRIPT ERROR|Parse Error|Failed to load script|ERROR:" "$LOG_DIR/survival_ten.log" | grep -v 'get_system_ca_certificates' | grep -v 'Condition "ret != noErr"' || true)"
if [[ -n "$filtered_errors" ]]; then
	printf '%s\n' "$filtered_errors" >&2
	exit 1
fi
...
filtered_errors="$(grep -E "SCRIPT ERROR|Parse Error|Failed to load script|ERROR:" "$LOG_DIR/survival_playthrough.log" | grep -v 'get_system_ca_certificates' | grep -v 'Condition "ret != noErr"' || true)"
if [[ -n "$filtered_errors" ]]; then
	printf '%s\n' "$filtered_errors" >&2
	exit 1
fi
